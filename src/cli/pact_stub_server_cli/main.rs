...
fn pact_source(matches: &ArgMatches) -> Vec<PactSource> {
                    ...
                    let auth = matches
                        ...
                        .map(|u| {
                            ...
                            HttpAuth::User(
                                ...
                                auth.next().map(|p| p.to_string()),
                            )
                        })
                        .or_else(|| {
                            matches
                                .get_one::<String>("token")
                                .map(|v| HttpAuth::Token(v.clone()))
                        });
    ...
    if let Some(url) = matches.get_one::<String>("broker-url") {
        ...
        debug!(
            "Loading all pacts from Pact Broker at {} using {} authentication",
            url,
            auth.clone()
        ...
        sources.push(PactSource::Broker {
            ...
            auth,
            consumers: matches
                .get_many::<Regex>("consumer-name")
                .unwrap_or_default()
                .into_iter()
                .cloned()
                .collect(),
            providers: matches
                .get_many::<Regex>("provider-name")
                .unwrap_or_default()
                .into_iter()
                .cloned()
                .collect(),
...
pub async fn handle_matches(matches: &ArgMatches) -> Result<(), ExitCode> {
    ...
    let pacts = load_pacts(
        sources,
        matches.get_flag("insecure-tls"),
        matches.get_one("ext"),
    )
    .await;
    if pacts.iter().any(|p| p.is_err()) {
        error!("There were errors loading the pact files.");
        for error in pacts.iter().filter(|p| p.is_err()).map(|e| match e {
            Err(err) => err.clone(),
        ...
        let provider_state_header_name = matches
            .get_one::<String>("provider-state-header-name")
            .cloned();
        ...
        info!(
            "Loaded {} pacts ({} total interactions)",
            pacts.len(),
            interactions
        );
        ...
        tokio::task::spawn_blocking(move || server_handler.start_server(port))
            .await
            .unwrap()
    }
}
