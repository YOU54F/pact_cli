...
use crate::cli::pact_stub_server_cli::main::{pact_support, PactSource};
...
impl ServerHandlerFactory {
    pub fn new(handler: ServerHandler) -> Self {
        ServerHandlerFactory { inner: handler }
    }
}
        ...
        match Server::try_bind(&addr) {
            Ok(builder) => {
                ...
                block_on(server).map_err(|err| {
                    error!(
                        "error occurred scheduling server future on Tokio runtime: {}",
                        err
                    );
                    ExitCode::from(2)
                })?;
...
async fn find_matching_request(
    ...
    let interactions = sources
        .iter()
        .flat_map(|(source, _)| {
            source
                .filter_interactions(V4InteractionType::Synchronous_HTTP)
                .iter()
        ...
        .filter(|(http, _)| {
            let path_context = CoreMatchingContext::new(
                ...
                &http
                    .request
                    .matching_rules
                    .rules_for_category("path")
                    .unwrap_or_default(),
                &hashmap! {},
            );
            pact_matching::match_method(&http.request.method, &request.method).is_ok()
                && pact_matching::match_path(&http.request.path, &request.path, &path_context)
                    .is_ok()
        })
        .filter(|(i, _)| {
            ...
            match provider_state {
                Some(ref regex) => {
                    empty_provider_states && ps.is_empty()
                        || ps.iter().any(|state| {
                            empty_provider_states && state.name.is_empty()
                                || regex.is_match(state.name.as_str())
                        })
                }
                None => true,
            }
    ...
    let results = futures::stream::iter(interactions)
        .filter_map(|(i, pact)| async move {
            ...
            let all_matched = mismatches.iter().all(|mismatch| match mismatch {
    ...
    match match_results.first() {
        Some((interaction, _)) => Ok(pact_matching::generate_response(
        ...
        None => {
            if auto_cors && request.method.to_uppercase() == "OPTIONS" {
                ...
                Err(anyhow!(
                    "No matching request found for path {}",
                    request.path
                ))
            }
        }
        ...
        Err(msg) => {
            ...
            if auto_cors {
                response.headers = Some(
                    hashmap! { "Access-Control-Allow-Origin".to_string() => vec!["*".to_string()] },
                )
            }
            response
        }
...
mod test {
    ...
    use crate::cli::pact_stub_server_cli::main::PactSource;
    ...
    async fn match_request_finds_the_most_appropriate_response() {
        ...
        expect!(
            super::find_matching_request(
                &request1,
                false,
                false,
                vec![(pact, PactSource::Unknown)],
                None,
                false
            )
            .await
    ...
    async fn match_request_excludes_put_or_post_requests_with_different_bodies() {
        ...
        let request4 = HttpRequest {
            method: "PUT".to_string(),
            headers: Some(
                hashmap! { "Content-Type".to_string() => vec!["application/json".to_string()] },
            ),
            ..HttpRequest::default()
        };
        ...
        expect!(
            super::find_matching_request(
                ...
                false,
                false,
                vec![(pact.clone(), PactSource::Unknown)],
                None,
                false
            )
            .await
    ...
    async fn match_request_returns_the_closest_match() {
        ...
        expect!(
            super::find_matching_request(
                ...
                false,
                false,
                vec![(pact1, PactSource::Unknown), (pact2, PactSource::Unknown)],
                None,
                false
            )
            .await
    ...
    async fn match_request_filters_interactions_if_provider_state_filter_is_provided() {
        ...
        let interaction3 = SynchronousHttp {
            provider_states: vec![
                ProviderState::default("state one"),
        ...
        let pact = V4Pact {
            interactions: vec![
                interaction1.boxed_v4(),
                interaction2.boxed_v4(),
                interaction3.boxed_v4(),
                interaction4.boxed_v4(),
            ],
            ..V4Pact::default()
        };
        ...
        expect!(
            super::find_matching_request(
                ...
                false,
                vec![(pact.clone(), PactSource::Unknown)],
                Some(Regex::new("state one").unwrap()),
                false
            )
            .await
        )
        .to(be_ok().value(response1.clone()));
        expect!(
            super::find_matching_request(
                &request,
                false,
                false,
                vec![(pact.clone(), PactSource::Unknown)],
                Some(Regex::new("state two").unwrap()),
                false
            )
            .await
        )
        .to(be_ok().value(response2.clone()));
        expect!(
            super::find_matching_request(
                &request,
                false,
                false,
                vec![(pact.clone(), PactSource::Unknown)],
                Some(Regex::new("state three").unwrap()),
                false
            )
            .await
        )
        .to(be_ok().value(response3.clone()));
        expect!(
            super::find_matching_request(
                &request,
                false,
                false,
                vec![(pact.clone(), PactSource::Unknown)],
                Some(Regex::new("state four").unwrap()),
                false
            )
            .await
        )
    ...
    async fn match_request_filters_interactions_if_provider_state_filter_is_provided_and_empty_values_included(
        ...
        expect!(
            super::find_matching_request(
                ...
                false,
                false,
                vec![(pact1, PactSource::Unknown)],
                Some(Regex::new("any state").unwrap()),
                true
            )
            .await
        )
    ...
    async fn handles_repeated_headers_values() {
        let interaction = SynchronousHttp {
            request: HttpRequest {
                headers: Some(hashmap! { "TEST-X".to_string() => vec!["X, Z".to_string()] }),
                ..HttpRequest::default()
            },
            response: HttpResponse {
                headers: Some(hashmap! { "TEST-X".to_string() => vec!["X, Y".to_string()] }),
                ..HttpResponse::default()
            },
            ..SynchronousHttp::default()
        };
        ...
        let result = super::find_matching_request(
            &request,
            false,
            false,
            vec![(pact, PactSource::Unknown)],
