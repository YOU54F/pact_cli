...
impl From<std::io::Error> for PactError {
    fn from(err: std::io::Error) -> Self {
        PactError {
            message: format!("Failed to load pact file: {}", err),
            path: None,
        }
    }
}
...
pub async fn load_pacts(
    ...
    futures::stream::iter(sources)
        .then(|s| async move {
            let values =
                match &s {
                    PactSource::File(file) => vec![read_pact(Path::new(file))
                        .map(|p| (p, s.clone()))
                        .map_err(PactError::from)],
                    PactSource::Dir(dir) => {
                        match walkdir(Path::new(dir), ext.unwrap_or(&"json".to_string()), &s) {
                            Ok(pacts) => pacts,
                            Err(err) => vec![Err(PactError::new(format!(
                                "Could not load pacts from directory '{}' - {}",
                                dir, err
                            )))],
                        }
                    }
                    PactSource::URL(url, auth) => vec![pact_from_url(url, auth, insecure_tls)
                        .await
                        .map(|p| (p, s.clone()))],
                    PactSource::Broker {
                        url,
                        ...
                        match client
                            .navigate("pb:latest-pact-versions", &hashmap! {})
                            .await
                        {
                            Ok(client) => match client.clone().iter_links("pb:pacts") {
                                Ok(links) => {
                                    futures::stream::iter(
                                        links.iter().map(|link| (link.clone(), client.clone())),
                                    )
                                    .then(|(link, client)| async move {
                                        client
                                            .clone()
                                            .fetch_url(&link, &hashmap! {})
                                            .await
                                            .map_err(|err| PactError::new(err.to_string()))
                                            .and_then(|json| {
                                                let pact_title =
                                                    link.title.clone().unwrap_or_else(|| {
                                                        link.href.clone().unwrap_or_default()
                                                    });
                                                debug!("Found pact {}", pact_title);
                                                load_pact_from_json(
                                                    link.href.clone().unwrap_or_default().as_str(),
                                                    &json,
                                                )
                                                .map_err(|err| {
                                                    PactError::new(format!(
                                                        "Error loading \"{}\" ({}) - {}",
                                                        pact_title,
...
fn filter_providers(
    providers: &Vec<Regex>,
    result: &Result<Box<dyn Pact + Send + Sync + RefUnwindSafe>, PactError>,
) -> Ready<bool> {
    match result {
        Ok(pact) => {
...
mod tests {
    ...
    use crate::cli::pact_stub_server_cli::main::loading::{
        filter_consumers, filter_providers, PactError,
    };
