...
fn extract_headers(headers: &HeaderMap<HeaderValue>) -> Option<HashMap<String, Vec<String>>> {
    if !headers.is_empty() {
        let result: HashMap<String, Vec<String>> = headers
            .keys()
            .map(|name| {
                let values = headers.get_all(name);
                let parsed_vals: Vec<Result<String, ()>> = values
                    .iter()
                    .map(|val| {
                        val.to_str().map(|v| v.to_string()).map_err(|err| {
...
pub fn pact_response_to_hyper_response(
...
mod test {
    ...
    fn test_response() {
        ...
        expect!(hyper_response.headers().get("Access-Control-Allow-Origin"))
            .to(be_some().value(HeaderValue::from_static("*")));
    }
    ...
    fn test_response_with_content_type() {
        let response = HttpResponse {
            status: 201,
            headers: Some(
                hashmap! { "Content-Type".to_string() => vec!["text/dizzy".to_string()] },
            ),
        ...
        expect!(hyper_response.headers().get("content-type"))
            .to(be_some().value(HeaderValue::from_static("text/dizzy")));
    }
    ...
    fn adds_a_content_type_if_there_is_not_one_and_there_is_a_body() {
        ...
        expect!(hyper_response.headers().is_empty()).to(be_false());
        expect!(hyper_response.headers().get("content-type"))
            .to(be_some().value(HeaderValue::from_static("application/json")));
    }
    ...
    fn only_add_a_cors_origin_header_if_one_has_not_already_been_provided() {
        let response = HttpResponse {
            headers: Some(
                hashmap! { "Access-Control-Allow-Origin".to_string() => vec!["dodgy.com".to_string()] },
            ),
            ..HttpResponse::default()
        };
        let hyper_response = pact_response_to_hyper_response(&response).unwrap();
