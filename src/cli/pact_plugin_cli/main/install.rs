...
use crate::cli::pact_plugin_cli::main::repository::{APP_USER_AGENT, DEFAULT_INDEX};
use crate::cli::pact_plugin_cli::main::{find_plugin, resolve_plugin_dir};
...
async fn install_known_plugin(
    ...
    if let Some(entry) = index.entries.get(name) {
        ...
        if let Some(version_entry) = entry.versions.iter().find(|v| v.version == version) {
            install_plugin_from_url(
                &http_client,
                version_entry.source.value().as_str(),
                override_prompt,
                skip_if_installed,
            )
            .await
            ...
            Err(anyhow!(
                "'{}' is not a valid version for plugin '{}'",
                version,
                name
            ))
        }
        ...
        Err(anyhow!(
            "'{}' is not a known plugin. Known plugins are: {}",
            name,
    ...
    if let Some(map) = response.as_object() {
        if let Some(tag) = map.get("tag_name") {
            ...
            if !skip_if_installed || !already_installed(&manifest) {
                ...
                download_plugin_executable(&manifest, &plugin_dir, &http_client, url, &tag, true)
                    .await?;
                ...
                println!(
                    "Skipping installing plugin {} version {} as it is already installed",
                    manifest.name, manifest.version
                );
                Ok(())
            }
...
pub(crate) async fn fetch_json_from_url(
    source: &str,
    http_client: &Client,
) -> anyhow::Result<Value> {
    ...
    let response: Value = http_client
        ...
        .send()
        .await
        .context("Fetching root document for source")?
        .json()
        .await
        .context("Parsing root JSON document for source")?;
...
fn prompt_continue(manifest: &PactPluginManifest) -> bool {
    let question = requestty::Question::confirm("overwrite_plugin")
        .message(format!(
            "Plugin with name '{}' and version '{}' already exists. Overwrite it?",
            manifest.name, manifest.version
        ))
        .default(false)
