...
use pact_plugin_driver::repository::{
    calculate_sha, get_sha_file_for_repository_file, load_index_file, load_sha, ManifestSource,
    PluginEntry, PluginRepositoryIndex,
};
...
pub(crate) fn handle_command(command: &ArgMatches) -> anyhow::Result<()> {
    match command.subcommand() {
        ...
        Some(("list-versions", args)) => list_versions(
            args.get_one::<String>("filename").unwrap(),
...
fn new_repository(filename: &Option<String>, overwrite: bool) -> anyhow::Result<()> {
    ...
    if path.exists() && !overwrite {
        Err(anyhow!(
            "Repository file '{}' already exists and overwrite was not specified",
        ...
        println!(
            "Created new blank repository file '{}'",
            abs_path.to_string_lossy()
        );
...
fn validate_repository_file(filename: &String) -> anyhow::Result<()> {
    ...
    if path.exists() {
        ...
        if sha != expected_sha {
            return Err(anyhow!(
                "Error: SHA256 digest does not match: expected {} but got {}",
                expected_sha,
                sha
            ));
        }
        ...
        if index.format_version != 0 {
            return Err(anyhow!(
                "Error: format_version is not valid: {}",
        ...
        table.add_row(vec![
            "Format Version",
            index.format_version.to_string().as_str(),
            "",
        ]);
        table.add_row(vec![
            "Index Version",
            index.index_version.to_string().as_str(),
            "",
        ]);
        ...
        table.add_row(vec![
            "Last Modified",
            index.timestamp.to_string().as_str(),
            additional.as_str(),
        ]);

        table.add_row(vec![
            "Plugin Entries",
            index.entries.len().to_string().as_str(),
            "",
        ]);
        let versions = index
            .entries
    ...
    match command.subcommand() {
        Some(("file", args)) => {
            ...
            add_manifest_to_index(
                ...
                &manifest,
                &mut index,
                &ManifestSource::File(file.to_string_lossy().to_string()),
            )?;
            Ok(())
        }
        Some(("git-hub", args)) => {
            ...
            add_manifest_to_index(
                ...
                &manifest,
                &mut index,
                &ManifestSource::GitHubRelease(url.to_string()),
            )?;
            Ok(())
        }
        Some((&_, _)) => Err(anyhow!("unrecognized subcommand is shown")),
...
fn add_manifest_to_index(
    ...
    println!(
        "Added plugin version {}/{} to repository file '{}'",
        manifest.name,
        manifest.version,
        repository_file.to_string_lossy()
    );
    Ok(())
}
...
fn write_index_file(
    ...
    if repository_file.exists() {
        ...
        if index.index_version <= existing.index_version {
            ...
            let mut f =
                File::create(repository_file).context("Could not open repository file to write")?;
            f.write_all(toml.as_bytes())
                .context("Failed to write to repository file")?;
        }
    } else {
    ...
    let result = runtime.block_on(async {
        ...
        if let Some(map) = response.as_object() {
            if let Some(tag) = map.get("tag_name") {
                ...
                let manifest_json =
                    download_json_from_github(&http_client, url, &tag, "pact-plugin.json")
                        .await
                        .context("Downloading manifest file from GitHub")?;
...
fn handle_add_all_versions(
    ...
    let source = format!(
        ...
        base_url
            .clone()
            .unwrap_or("https://api.github.com/repos".to_string()),
        owner,
        repository
    );
    ...
    let result = runtime.block_on(async move {
        ...
        match response {
            Value::Array(values) => {
                for value in values {
                    match value {
                        Value::Object(attributes) => {
                            if let Some(url) = attributes.get("html_url") {
                                ...
                                if let Some(assets) = attributes.get("assets") {
                                    if let Value::Array(assets) = assets {
                                        if let Some(manifest_url) =
                                            assets.iter().find_map(|item| match item {
                                                Value::Object(attributes) => {
                                            ...
                                            })
                                        {
                                            ...
                                            let manifest: PactPluginManifest =
                                                serde_json::from_value(manifest_json).context(
                                                    "Parsing JSON manifest file from GitHub",
                                                )?;
                                            index
                                                .entries
                                                .entry(manifest.name.clone())
                                                .and_modify(|entry| {
                                                    entry.add_version(&manifest, &source)
                                                })
                                                .or_insert_with(|| {
                                                    PluginEntry::new(&manifest, &source)
                                                });
                                        }
                                    }
                                }
                ...
                index.index_version += 1;
                write_index_file(&repository_file, &index)
                    .context("Failed to write updated index file")?;
                generate_sha_digest(&repository_file)?;

                println!(
                    "Added all plugin versions from {}/{} to repository file '{}'",
                    owner,
                    repository,
                    repository_file.to_string_lossy()
                );
...
mod tests {
    ...
    use crate::cli::pact_plugin_cli::main;
    use main::repository::{
        add_manifest_to_index, load_index_file, new_repository, ManifestSource,
    ...
    fn add_manifest_to_index_test() {
        ...
        let result = add_manifest_to_index(
            &path.to_path_buf(),
            &manifest,
            &mut index,
            &ManifestSource::File("Test".to_string()),
        );
