...
pub async fn create_mock_server(
    ...
    match RequestResponsePact::read_pact(Path::new(file)) {
            ...
            let json = match pact.to_json(pact.specification_version()) {
                ...
                Err(err) => {
                    crate::cli::pact_mock_server_cli::main::display_error(
                        format!("Failed to send pact as JSON '{}': {}", file, err),
                        usage,
                    );
                    return Err(2);
                }
            };
            let resp = client.post(url.as_str()).json(&json).send().await;
            match resp {
                Ok(response) => {
                  ...
                  .as_u64().ok_or_else(|| handle_error("Invalid JSON received from master server - mockServer port attribute is not a number"))?;
                        ...
                        crate::cli::pact_mock_server_cli::main::display_error(
                            format!(
                                "Master mock server returned an error: {}\n{}",
                                response.status(),
        ...
        Err(err) => {
            crate::cli::pact_mock_server_cli::main::display_error(
                format!("Failed to load pact file '{}': {}", file, err),
                usage,
            );
