...
use pact_mock_server::{mock_server::MockServer, server_manager::ServerManager};

use crate::cli::pact_mock_server_cli::main::handle_error;
pub async fn verify_mock_server(
    host: &str,
    port: u16,
    matches: &ArgMatches,
    usage: &str,
    ...
    let (id, id_type) = match (mock_server_id, mock_server_port) {
        ...
        _ => {
            crate::cli::pact_mock_server_cli::main::display_error(
                "Either an ID or port must be provided".to_string(),
                usage,
            );
            Err(2)?
        }
    ...
    match resp {
        Ok(result) => {
            ...
            if !status.is_success() {
                match status {
                    ...
                    StatusCode::UNPROCESSABLE_ENTITY => match result.text().await {
                        Ok(body) => match serde_json::from_str::<Value>(body.as_str()) {
                            Ok(json) => {
                    ...
                    _ => {
                        crate::cli::pact_mock_server_cli::main::display_error(
                            format!(
                                "Unexpected response from master mock server '{}': {}",
                                url,
                                result.status()
                            ),
                            usage,
        ...
        Err(err) => {
            crate::cli::pact_mock_server_cli::main::display_error(
                format!(
                    "Failed to connect to the master mock server '{}': {}",
                    url, err
                ),
                usage,
            );
...
fn validate_port(port: u16, server_manager: &Mutex<ServerManager>) -> Result<MockServer, String> {
    server_manager
        .lock()
        .unwrap()
        .find_mock_server_by_port_mut(port, &|ms| ms.clone())
...
fn validate_uuid(id: &str, server_manager: &Mutex<ServerManager>) -> Result<MockServer, String> {
    server_manager
        .lock()
        .unwrap()
        .find_mock_server_by_id(&id.to_string(), &|_, ms| ms.unwrap_left().clone())
...
fn display_verification_errors(id: &str, port: u64, json: &serde_json::Value) {
    ...
    println!(
        "Mock server {}/{} failed verification with {} errors\n",
        id,
        port,
        mismatches.len()
    );
    ...
    for (i, mismatch) in mismatches.iter().enumerate() {
        match json_to_string(mismatch.get("type").unwrap()).as_str() {
            ...
            "request-not-found" => {
                let request = mismatch.get("request").unwrap();
                println!(
                    "{} - Received a request that was not expected - {}",
                    i, request
                )
            }
            "request-mismatch" => {
                ...
                println!(
                    "{} - Received a request that did not match with expected - {} {}",
                    i, method, path
                );
                ...
                for request_mismatch in request_mismatches {
                    println!(
                        "        {}",
                        request_mismatch.get("mismatch").unwrap().to_string()
                    )
                }
            }
